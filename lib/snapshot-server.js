...
SnapshotServer.prototype = _.extend(SnapshotServer.prototype, {
	_start: function() {
		...
		this._startZmqServer();
	},

	_startZmqServer: function() {
		var that = this;
		var zmqSocket = this._zmqSocket = zmq.socket('rep');
		zmqSocket.identity = 'server' + process.pid;
		zmqSocket.bind('tcp://127.0.0.1:' + this._opt.zmqPort, function(err) {
			if(err) {
				throw err;
			}
			console.log('ZMQ Server listening on port ' + that._opt.zmqPort);
			zmqSocket.on('message', function(msg) {
				msg = JSON.parse(msg.toString());
				if(that._zmqMessageHandler[msg.type]) {
					that._zmqMessageHandler[msg.type].call(that, msg);
				} else {
					console.log('ZMQ Server unknown message type: ' + msg.type);
				}
			});
		});
	},
	...
	_startWsServer: function() {
		...
		setTimeout(function() {
			...
			for(var i = 0; i < 10; i++) {
				...
				if(!req.storagePath) {
					req.storagePath = path.join(config.storageBasePath, encodeURIComponent(req.url)) + '.png';
				}
				that._dispatchTask(req, function(res) {
	...
	_zmqMessageHandler: {
		snapshot: function(msg) {
			var that = this;
			var task = msg.task;
			var storagePath = task.storagePath;
			if(!storagePath) {
				storagePath = encodeURIComponent(task.url) + '.png';
			}
			task.storagePath = path.join(config.storageBasePath, storagePath);
			this._dispatchTask(task, function(res) {
				console.log(task.url + ' ' + res.status);
				that._zmqSocket.send(JSON.stringify({
